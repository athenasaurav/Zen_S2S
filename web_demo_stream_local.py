...
# SSL certificate generation and usage removed for local version


def get_args():
    parser = argparse.ArgumentParser(description="VITA-Audio (Local HTTP)")
    ...
    for new_text in streamer:
        ...
        if "<|end_of_audio|>" == new_text:
            ...
            if sid is not None:
                ...
                socketio.emit("audio", output_data.tobytes(), to=sid)
            if num_audio_chunk == 0:
                first_audio_time = (
            ...
            if len(audio_tokens) > 512:
                ...
                past_audio_token_len = 0
            num_audio_chunk += 1
    # After streaming all audio, notify client
    if sid is not None:
        socketio.emit("audio_end", {}, to=sid)
...
if __name__ == "__main__":
    logger.info("Start VITA-Audio server (HTTP, local)")
    logger.info("=" * 100)
    logger.info("Warmup...")
    ...
    logger.info("=" * 100)
    logger.info("=" * 100)
    logger.info("Warmup second run...")
    run_infer_stream("asset/sample_001.wav", None)
    logger.info("Warmup second run Done.")
    logger.info("=" * 100)
    logger.info("=" * 100)
    logger.info("Warmup third run...")
    run_infer_stream("asset/sample_002.wav", None)
    logger.info("Warmup third run Done.")
    logger.info("=" * 100)
    logger.info("=" * 100)
    logger.info("Warmup fourth run...")
    run_infer_stream("asset/sample_003.wav", None)
    logger.info("Warmup fourth run Done.")
    logger.info("=" * 100)
    logger.info("=" * 100)
    logger.info("Warmup fifth run...")
    run_infer_stream("asset/sample_004.wav", None)
    logger.info("Warmup fifth run Done.")
    logger.info("=" * 100)
    socketio.run(app, host=args.ip, port=int(args.port)) 