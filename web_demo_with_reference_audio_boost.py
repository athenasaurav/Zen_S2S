...
def _launch_demo(model, tokenizer, audio_tokenizer):
    def predict(_chatbot, task_history, task, reference_audio):
        ...
        if reference_audio is not None and os.path.exists(reference_audio):
            ...
            # Add reference audio to the system message for voice cloning
            system_message = {
                "role": "system",
                "content": f"Your Voice: <|audio|>",
            }
            # Add reference audio to the audio path list for processing
            audio_path_list.append(reference_audio)
            messages.append(system_message)

        if task == "Spoken QA":
            if not messages:  # Only add default system message if no reference audio
                messages = [
                    {
                        "role": "system",
                ...
                # Append the default system content to the existing system message with reference audio
                messages[0]["content"] += "\nYour Name: Lucy\nYour Gender: female\nRespond in a text-audio interleaved manner."
            ...
            if not messages:  # Only add if no reference audio system message
                messages = []
        ...
        audio_offset = tokenizer.convert_tokens_to_ids("<|begin_of_audio|>")
    ...
    with gr.Blocks(title="VITA-Audio-Boost with Reference Audio") as demo:
        gr.Markdown("""<center><font size=8>VITA-Audio-Boost with Reference Audio</center>""")
        gr.Markdown(
            """<center><font size=4>Enhanced deployment of the VITA-Audio-Boost model with reference audio support for voice cloning. Upload a reference audio file to clone the voice characteristics in the generated responses. The currently deployed model is VITA-Audio-Boost (optimized for maximum performance). For the ASR and TTS tasks, only single-turn dialogues are supported. In the Spoken QA task, generated text is used as dialogue history to reduce the context length.</center>"""
        )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                reference_audio = gr.Audio(
                    ...
                    type="filepath",
                    label="🎯 Upload Reference Audio (Optional)",
                    info="Upload a 10-second audio sample to clone the voice characteristics. Leave empty to use default voice.",
                    show_download_button=True,
                    waveform_options=gr.WaveformOptions(sample_rate=16000),
                )
                gr.Markdown("*Tip: For best results, use clear speech without background noise, approximately 10 seconds long.*")
        
        chatbot = gr.Chatbot(
            label="VITA-Audio-Boost", elem_classes="control-height", height=500
        )
        ...
        # Update the click handlers to include reference_audio parameter
        add_text_button.click(
            ...
            predict, [chatbot, task_history, task, reference_audio], [chatbot, audio_output], show_progress=True
        )
        ...
        add_audio_button.click(
            ...
            show_progress=True,
        ).then(predict, [chatbot, task_history, task, reference_audio], [chatbot, audio_output], show_progress=True)
...
def main():
    ...
    # Updated to use VITA-Audio-Boost model
